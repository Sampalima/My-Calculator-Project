...
class Calculator:
    def __init__(self, master):
        ...
        master.geometry('350x450+0+0')
        master.config(bg='black')
        master.resizable(False, False)

        self.equation = StringVar()  # nitializes self.equation with a StringVar to display the input and result
        self.entry_value = ''
        Entry(width=18, bg='#ccddff', font=('Times New Roman', 30), textvariable=self.equation).place(x=0,
                                                                                                 y=0)  # The Entry widget displays the current input and result at the top
